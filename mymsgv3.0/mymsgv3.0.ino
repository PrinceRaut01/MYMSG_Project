...
int scrollY = 0;                 // current vertical offset in pixels
const int lineHeight = 10;       // font height
const int maxLinesOnScreen = 4;  // OLED can show 4 lines of 6x10 font
bool needScroll = false;
unsigned long lastScroll = 0;
unsigned long scrollInterval = 150; // default scroll speed in ms
...
<div class="container">
  ...
  <div style="margin-top:20px;">
    ...
    <input type="range" id="speed" name="speed" min="50" max="1000" step="50" value="150" 
...
// Draw messages with smooth vertical scrolling
void drawMessages() {
  u8g2.clearBuffer();
  u8g2.setFont(u8g2_font_6x10_tr);
...
void handleSpeed() {
  if (server.hasArg("value")) {
    scrollInterval = server.arg("value").toInt();
    if (scrollInterval < 50) scrollInterval = 50;
    if (scrollInterval > 1000) scrollInterval = 1000;
  }
...
void loop() {
  ...
  delay(50); // fast refresh for smooth scrolling
}
